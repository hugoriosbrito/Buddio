...
pub fn validate_backup_archive(bytes: &[u8]) -> Result<BackupManifest> {
    ...
    for i in 0..archive.len() {
        ...
        if name.starts_with('/') || name.contains("..") || name.contains('\\') {
            bail!("backup contains unsafe archive path");
        }
        total = total.saturating_add(entry.size());
        if total > MAX_EXTRACTED_BYTES {
            bail!("backup exceeds extracted size limit");
        }
    ...
    if manifest.kind != "buddio-backup" || manifest.version != BACKUP_FORMAT_VERSION {
        bail!("backup format is not supported");
    }
    Ok(manifest)
}
...
mod tests {
    ...
    fn accepts_versioned_buddio_backup() {
        let bytes = archive(&[(
            ...
            r#"{"version":1,"kind":"buddio-backup"}"#,
        )]);
    ...
    fn creates_a_valid_backup_archive() {
        ...
        assert_eq!(
            validate_backup_archive(&bytes).unwrap().kind,
            "buddio-backup"
        );
    }
}
